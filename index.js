...
function customAssign(...objects) {
  if (Object.keys(objects).length >= 2) {
    for (const item in objects) {
      const num = objects[item];

      for (const item2 in num) {
        if (typeof num[item2] === "number") {
          num[item2] = num[item2].toString();
        }
      }
    }
...
function filterObjectsByKey(arr, key) {
  if (Array.isArray(arr) === true) {
    const filt = arr.filter((obj) => (obj.hasOwnProperty(key) ? obj : null), 0);
    return filt;
  } else {
    return;
  }
...
function sealAndCheck(obj) {
  if (typeof obj === "object") {
    const check = Object.seal(obj);
    const check2 = Object.isSealed(check);
    return check2;
    // Object.seal(obj);
...
function checkOwnership(obj1, obj2, key) {
  if (obj1.hasOwnProperty(key) === true && obj2.hasOwnProperty(key) === true) {
    return true;
  } else {
    return false;
  }
...
function getObjectValuesSum(obj) {
  if (typeof obj === "object") {
    const get = Object.values(obj);
    const num = get.reduce((num1, num2) => num1 + num2, 0);
    return num;
  } else {
    return 0;
  }
